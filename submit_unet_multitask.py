...
if MODE == 'LOCAL':
    model_parameter_path = './binary_classifier_unet_multitask.pth'
...
def predict(video_path):
    ...
    with torch.no_grad():
        y = model.forward_classifier(x.to(gpu))
        y = torch.sigmoid(y.squeeze()).mean().item()
...
checkpoint = torch.load('binary_classifier_unet_multitask.pth', map_location=gpu)
